...
import { guessFileThumbnail, isPdf, isStreamableVideo, isTxt, resolveSize } from 'src/common/file';
...
function preview(u, dialog, nav, fileService, isMobile, onNav = null): void {
  ...
  fileService.generateFileTempToken(u.fileKey, isStreaming ? TokenType.STREAMING : TokenType.DOWNLOAD)
    .subscribe({
      next: (resp) => {
...
<div mat-dialog-content>
    ...
    <p class="mb-3"><b>Accumulated size: '{{totalSizeLabel}}'</b></p>

      <table mat-table [dataSource]="tabdata" class="mat-elevation-z8 mb-4" style="width: 100%;" multiTemplateDataRows>
        ...
        <ng-container matColumnDef="fileKey">
            ...
            <td mat-cell *matCellDef="let u" (click)="$event.stopPropagation()"> {{u.fileKey}} </td>
        </ng-container>

        <ng-container matColumnDef="thumbnail">
            <th mat-header-cell *matHeaderCellDef> <b>Preview</b> </th>
            <td mat-cell *matCellDef="let f" (click)="preview(f)">
                <img style="max-height:50px; padding: 5px 0px 5px 0px;" *ngIf="f.thumbnail" [src]="f.thumbnail" />
                <img style="max-height:40px; padding: 5px 0px 5px 0px;" *ngIf="!f.thumbnail"
        ...
        <ng-container matColumnDef="operate">
            ...
            <td mat-cell *matCellDef="let f">
              <button class="small-btn m-2" mat-raised-button (click)="$event.stopPropagation() || jumpToDownloadUrl(f.fileKey)">
...
export class VerFileHistoryComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<VerFileHistoryComponent, VerFileHistoryDialogData>,
  ...
  qryTotalSize() {
    this.http.post<any>(environment.vfm, '/open/api/versioned-file/accumulated-size', { verFileId: this.data.verFileId }, false).
      subscribe({
        next: (r) => {
  ...
  fetch() {
    this.http.post<any>(environment.vfm, '/open/api/versioned-file/history', { paging: this.pagingController.paging, verFileId: this.data.verFileId }, false).
      subscribe({
        next: (r) => {
          if (!r.data.payload) {
            r.data.payload = [];
          ...
          for (let f of this.tabdata) {
            if (f.thumbnail) {
              f.thumbnail = environment.fstore + "/file/raw?key=" + encodeURIComponent(f.thumbnail);
            }
...
@Component({
  ...
  <div *ngIf="expandUploadPanel" class="container-fluid bootstrap p-3 shadow rounded mt-3 border">

      <ng-container *ngIf="!updateVerFileId">
        <h4 class="mt-3 mb-3">Create Versioned File</h4>
      </ng-container>
      <ng-container *ngIf="updateVerFileId">
        <h4 class="mt-3 mb-3">Update Versioned File {{updateVerFileId}} {{updateVerFileName}}</h4>
      </ng-container>

      <mat-form-field style="width: 100%;" class="mt-1 mb-1">
          <mat-label>Name</mat-label>
          <input matInput type="text" [(ngModel)]="uploadFileName" [disabled]="isUploading">
          <button *ngIf="uploadFileName" matSuffix aria-label="Clear" (click)="uploadFileName=''"
              class="btn-close" [disabled]="isUploading">
          </button>
      </mat-form-field>
      ...
      <div class="input-group input-group-lg mt-1 mb-1">
          <input type="file" class="form-control" #uploadFileInput (change)="onFileSelected($event.target.files)"
              aria-describedby="basic-addon1" [disabled]="isUploading">
      </div>
  ...
  <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-3">
      <button mat-raised-button class="m-2" [class.status-green]="expandUploadPanel" (click)="toggleUploadPanel()">Upload Panal</button>
  ...
  <div class="mt-3 mb-2" style="overflow: auto;">
      <table mat-table [dataSource]="tabdat" style="width: 100%;">
        ...
        <ng-container matColumnDef="verFileId">
            ...
            <td mat-cell *matCellDef="let u" (click)="$event.stopPropagation()"> {{u.verFileId}} </td>
        </ng-container>
        ...
        <ng-container matColumnDef="operate">
            <th mat-header-cell *matHeaderCellDef> <b>Operate</b></th>
            <td mat-cell *matCellDef="let f">
              <button mat-raised-button class="m-2" (click)="selectVerFile(f)"> Update </button>
              <button mat-raised-button class="m-2" (click)="showVerFileHistory(f)"> History </button>
           </td>
        </ng-container>
...
export class VersionedFileComponent implements OnInit {
  ...
  fetch() {
    ...
    this.http.post<any>(environment.vfm, '/open/api/versioned-file/list', req, false).
      subscribe({
        next: (r) => {
          if (!r.data.payload) {
            r.data.payload = [];
  ...
  upload() {
    ...
    this.uploadSub = this.fileService.uploadToMiniFstore({
